 ...
 *
 * This governor is a combination of interactive and conservative. The base
 * used was interactive. Conservative settings were added in.
 *
 */
...
#define DEF_FREQUENCY_UP_THRESHOLD		(15)
...
struct cpufreq_ragingeagles_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_ragingeagles_cpuinfo, cpuinfo);
...
static int cpufreq_governor_ragingeagles(struct cpufreq_policy *policy,
		unsigned int event);
...
struct cpufreq_governor cpufreq_gov_ragingeagles = {
	.name = "ragingeagles",
	.governor = cpufreq_governor_ragingeagles,
...
static void cpufreq_ragingeagles_timer_resched(
	struct cpufreq_ragingeagles_cpuinfo *pcpu)
{
 ...
 * function.
 */
static void cpufreq_ragingeagles_timer_start(int cpu)
{
	struct cpufreq_ragingeagles_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq(
	struct cpufreq_ragingeagles_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_ragingeagles_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	...
	struct cpufreq_ragingeagles_cpuinfo *picpu;
	...
	if (pcpu->target_freq >= hispeed_freq &&
		...
		trace_cpufreq_ragingeagles_notyet(
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_ragingeagles_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		goto rearm_if_notmax;
	}

	trace_cpufreq_ragingeagles_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_ragingeagles_timer_resched(pcpu);
...
static void cpufreq_ragingeagles_idle_start(void)
{
	struct cpufreq_ragingeagles_cpuinfo *pcpu =
...
static void cpufreq_ragingeagles_idle_end(void)
{
	struct cpufreq_ragingeagles_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_ragingeagles_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_ragingeagles_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_ragingeagles_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
			...
			if (max_freq != pcpu->policy->cur)
				__cpufreq_driver_target(pcpu->policy,
							max_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_ragingeagles_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
...
static void cpufreq_ragingeagles_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static int cpufreq_ragingeagles_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_ragingeagles_notifier,
};
			   ...
			   const char *buf, size_t count)
{
	...
	if (boost_val) {
		trace_cpufreq_ragingeagles_boost("on");
		cpufreq_ragingeagles_boost();
	} else {
		boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_ragingeagles_unboost("off");
	}
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_ragingeagles_boost("pulse");
	cpufreq_ragingeagles_boost();
	return count;
}
...
static struct attribute *ragingeagles_attributes[] = {
...
static struct attribute_group ragingeagles_attr_group = {
	.attrs = ragingeagles_attributes,
	.name = "ragingeagles",
};

static int cpufreq_ragingeagles_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_ragingeagles_idle_start();
		break;
	case IDLE_END:
		cpufreq_ragingeagles_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_ragingeagles_idle_nb = {
	.notifier_call = cpufreq_ragingeagles_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_ragingeagles_timer_start(j);
		...
		rc = sysfs_create_group(cpufreq_global_kobject,
				&ragingeagles_attr_group);
		if (rc) {
			mutex_unlock(&gov_lock);
			return rc;
		}

		idle_notifier_register(&cpufreq_ragingeagles_idle_nb);
		cpufreq_register_notifier(
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_ragingeagles_idle_nb);
		sysfs_remove_group(cpufreq_global_kobject,
				&ragingeagles_attr_group);
		mutex_unlock(&gov_lock);
...
static void cpufreq_ragingeagles_nop_timer(unsigned long data)
{
}

static int __init cpufreq_ragingeagles_init(void)
{
	unsigned int i;
	struct cpufreq_ragingeagles_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_ragingeagles_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_ragingeagles_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_ragingeagles_speedchange_task, NULL,
			       "cfragingeagles");
...
fs_initcall(cpufreq_ragingeagles_init);
#else
module_init(cpufreq_ragingeagles_init);
#endif

static void __exit cpufreq_ragingeagles_exit(void)
{
...
module_exit(cpufreq_ragingeagles_exit);
