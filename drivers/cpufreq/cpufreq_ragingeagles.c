...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_RAGINGVENTURES
static
#endif
...
{
	dbs_wq = alloc_workqueue("ragingventures_dbs_wq", WQ_HIGHPRI, 0);
	if (!dbs_wq) {
		printk(KERN_ERR "Failed to create ragingventures_dbs_wq workqueue\n");
		return -EFAULT;
	}
...
MODULE_DESCRIPTION("'cpufreq_ragingventures' - A dynamic cpufreq governor for Venturization of your mind. "
		"Low Latency Frequency Transition capable processors "
		"optimised for use in a battery environment. ¯\_(ツ)_/¯");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_RAGINGVENTURES
